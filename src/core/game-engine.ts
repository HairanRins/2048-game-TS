...
import type { IGameEngine, IEventEmitter } from "../types/interfaces.js";
import {
  BOARD_SIZE,
  INITIAL_TILES_COUNT,
...
export class GameEngine implements IGameEngine {
  ...
  private readonly eventEmitter: IEventEmitter<GameEvent>;
  private readonly tileGenerator: TileGenerator;
  private readonly boardValidator: BoardValidator;

  constructor(eventEmitter?: IEventEmitter<GameEvent>) {
    this.eventEmitter = eventEmitter || new EventEmitter<GameEvent>();
  ...
  move(direction: Direction): boolean {
    ...
    if (scoreGain > 0) {
      ...
      this.eventEmitter.emit("SCORE_UPDATED", {
        type: "SCORE_UPDATED",
        payload: { score: this.score.current },
      });
    }
    ...
    if (this.boardValidator.checkWinCondition(this.board)) {
      this.state = "won";
      this.eventEmitter.emit("GAME_WON", {
        type: "GAME_WON",
        payload: { score: this.score.current },
      });
      this.eventEmitter.emit("GAME_OVER", {
        type: "GAME_OVER",
        payload: { state: "won", score: this.score.current },
      });
    } else if (!this.boardValidator.hasValidMoves(this.board)) {
      this.state = "lost";
      this.eventEmitter.emit("GAME_LOST", {
        type: "GAME_LOST",
        payload: { score: this.score.current },
      });
      this.eventEmitter.emit("GAME_OVER", {
        type: "GAME_OVER",
        payload: { state: "lost", score: this.score.current },
      });
    }
  ...
  addRandomTile(): void {
    ...
    if (newBoard !== this.board) {
      ...
      const addedTile = newBoard.find(
        (newTile, idx) => oldBoard[idx]?.value === 0 && newTile.value !== 0,
      );
      this.board = newBoard;
      if (addedTile) {
  ...
  canMove(): boolean {
    return (
      this.state === "playing" && this.boardValidator.hasValidMoves(this.board)
    );
  }

  on(event: string, listener: (data: GameEvent) => void): void {
    this.eventEmitter.on(event, listener);
  }

  off(event: string, listener: (data: GameEvent) => void): void {
    this.eventEmitter.off(event, listener);
  }

  private performMove(board: Board, direction: Direction): Board {
    let newBoard = [...board];
  ...
  private processRow(row: readonly TileValue[]): readonly TileValue[] {
  ...
  private mergeRow(row: readonly TileValue[]): TileValue[] {
    ...
    for (let i = 0; i < row.length; i++) {
      ...
      if (
        ...
        result.push((current * 2) as TileValue);
        i++;
  ...
  private calculateScoreGain(previousBoard: Board, newBoard: Board): number {
    let scoreGain = 0;
